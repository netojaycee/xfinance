...
import  { useState } from "react";
  ...
  export default function StoreItemForm({ item, isEditMode = false }: { item?: any; isEditMode?: boolean; }) {
        ...
        <Tabs value={tab} onValueChange={handleTabChange} className="mb-4">
          ...
          <TabsContent value="product">
            {(!isEditMode || tab === "product") && (
              <StoreItemProductForm item={item} isEditMode={isEditMode && tab === "product"} />
            )}
          </TabsContent>
          <TabsContent value="service">
            {(!isEditMode || tab === "service") && (
              <StoreItemServiceForm item={item} isEditMode={isEditMode && tab === "service"} />
            )}
          </TabsContent>
        </Tabs>
      </div>
