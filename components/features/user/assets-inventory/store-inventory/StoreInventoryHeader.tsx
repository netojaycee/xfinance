  ...
  return (
    <div className="mb-4">
        <div className="flex items-start justify-between mb-4">
            <div>
                <h2 className="text-2xl font-bold text-indigo-900">Store Inventory</h2>
                <p className="text-muted-foreground">Manage office supplies and inventory</p>
            </div>
            <div className="flex items-center gap-3">
                ...
                <Button onClick={() => openModal(MODAL.SUPPLY_CREATE)} className="rounded-xl">
                    <Plus /> Add Supply
                </Button>
            </div>
        </div>
        ...
        <div className="flex items-center bg-yellow-50 border border-yellow-200 rounded-lg px-4 py-3 text-yellow-800">
          <span className="mr-3">
            <svg width="24" height="24" fill="none" viewBox="0 0 24 24">
              <path d="M12 9v4m0 4h.01M21 19V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2Z" stroke="#F59E42" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </span>
          <div>
            <span className="font-semibold">Stock Alert:</span> {stats.lowStock} item(s) are low on stock and {stats.outOfStock} item(s) are out of stock.
            <br />
            <span className="text-sm text-muted-foreground">Review inventory levels and place orders to restock supplies.</span>
          </div>
        </div>
      </div>
      {/* Stat Cards */}
      ...
      <CustomModal
        ...
        onOpenChange={(open) => open ? openModal(MODAL.SUPPLY_CREATE) : closeModal(MODAL.SUPPLY_CREATE)}
      >
        <StoreSupplyForm />
      </CustomModal>
    </div>
