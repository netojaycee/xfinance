...
export function EntitySwitcher({ entities, isLoading }: EntitySwitcherProps) {
  const [selectedEntity, setSelectedEntity] = React.useState<string | undefined>();
  const { entity: currentEntity } = useSessionStore();

  const { mutate: impersonateEntity, isPending: isImpersonating } = useImpersonateEntity({
    onSuccess: () => {
  ...
  React.useEffect(() => {
    if (currentEntity?.entityId && entities.some(e => e.id === currentEntity.entityId)) {
      ...
      if (!currentEntity?.entityId) {
        impersonateEntity({ entityId: firstEntity.id, entityName: firstEntity.name });
      }
    }
