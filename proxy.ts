...
const isProtected = protectedPaths.some((path) => {
  ...
  if (isProtected) {
    ...
    if (
      ...
      if (pathname === `/${pathSegment}` && Array.isArray(required) && required.length > 0) {
        ...
        if (submodule) {
          const redirectUrl = new URL(`/${pathSegment}/${submodule}`, request.url);
    ...
    if (user.systemRole === ENUM_ROLE.USER) {
      ...
      if (required) {
        ...
        if (pathname === `/${pathSegment}` && Array.isArray(required) && firstPermittedSubmodule) {
          // Map permission to submodule route
          // Example: 'sales:customers:view' => 'customers', etc.
          // You may want a more robust mapping if submodule names differ
          let submodule = '';
          if (firstPermittedSubmodule.includes(':')) {
            // e.g., 'sales:customers:view' => 'customers'
