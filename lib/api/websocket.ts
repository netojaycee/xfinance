...
import { RealtimeEventType, RealtimeEventPayload } from '@/lib/types/realtimeEvents';
...
export function initializeWebSocket(config: WebSocketConfig): Socket {
  ...
  const wsUrl = process.env.NEXT_PUBLIC_WS_URL || 'http://localhost:3810/cache';
...
function setupEventListeners(socket: Socket) {
  socket.on('whoami-invalidated', (data: RealtimeEventPayload['whoami-invalidated']) => {
  ...
  socket.on('permissions-changed', (data: RealtimeEventPayload['permissions-changed']) => {
    window.dispatchEvent(
      new CustomEvent('realtime-event', {
  ...
  socket.on('menus-invalidated', (data: RealtimeEventPayload['menus-invalidated']) => {
    window.dispatchEvent(
      new CustomEvent('realtime-event', {
  ...
  socket.on('entity-removed', (data: RealtimeEventPayload['entity-removed']) => {
    window.dispatchEvent(
      new CustomEvent('realtime-event', {
  ...
  socket.on('entity-updated', (data: RealtimeEventPayload['entity-updated']) => {
    window.dispatchEvent(
      new CustomEvent('realtime-event', {
  ...
  socket.on('user-role-changed', (data: RealtimeEventPayload['user-role-changed']) => {
    window.dispatchEvent(
      new CustomEvent('realtime-event', {
  ...
  socket.on('subscription-changed', (data: RealtimeEventPayload['subscription-changed']) => {
    window.dispatchEvent(
      new CustomEvent('realtime-event', {
  ...
  socket.on('subscription-expired', (data: RealtimeEventPayload['subscription-expired']) => {
    window.dispatchEvent(
      new CustomEvent('realtime-event', {
